...
export class AuditTrail {
  ...
  private maxEvents: number = 10000; // Configurable limit
    ...
    if (filters) {
      ...
      if (filters.complianceStatus) {
        filtered = filtered.filter(e =>
          e.compliance?.status === filters.complianceStatus
        );
      }
    }
  ...
  getAuditMetrics(): {
    ...
    for (const event of this.events) {
      ...
      if (event.compliance?.status === 'violation') violations++;
      if (event.compliance?.status === 'warning') warnings++;
