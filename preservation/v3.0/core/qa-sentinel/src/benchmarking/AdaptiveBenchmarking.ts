import { BenchmarkResult, PerformanceMetrics, BenchmarkComparison, Anomaly, TelemetryData } from '../types';
...
export class AdaptiveBenchmarking {
  ...
  async runBenchmark(buildId: string, telemetryData: TelemetryData[]): Promise<BenchmarkResult> {
  ...
  private async collectCurrentMetrics(telemetryData: TelemetryData[]): Promise<PerformanceMetrics> {
    // Group telemetry by component
    const componentData = new Map<string, TelemetryData[]>();
  ...
  private compareWithBaseline(current: PerformanceMetrics, baseline: PerformanceMetrics): BenchmarkComparison {
    ...
    const regressionDetected = (
      current.responseTime.avg > baseline.responseTime.avg * 1.1 || // 10% degradation
    ...
    if (delta.responseTime > 5) improvementAreas.push('Response time optimization needed');
    if (delta.errorRate > 10) improvementAreas.push('Error rate reduction required');
    if (delta.throughput < -5) improvementAreas.push('Throughput performance degraded');
  ...
  private detectBenchmarkAnomalies(current: PerformanceMetrics, baseline: PerformanceMetrics): Anomaly[] {
  ...
  getHistoricalResults(limit: number = 10): BenchmarkResult[] {
  ...
  getPerformanceTrend(component: string, days: number = 7): { date: Date, metrics: PerformanceMetrics }[] {
  ...
  isPerformanceAcceptable(metrics: PerformanceMetrics, baseline: PerformanceMetrics, threshold: number = 80): boolean {
