...
export class KubernetesAutoscaler {
  ...
  async scaleDeployment(namespace: string, deploymentName: string, decision: ScalingDecision): Promise<void> {
    try {
  ...
  async getDeploymentStatus(namespace: string, deploymentName: string): Promise<any> {
    try {
  ...
  async checkZeroDowntimeScaling(namespace: string, deploymentName: string): Promise<boolean> {
    try {
  ...
  async getResourceUtilization(namespace: string, deploymentName: string): Promise<{
    cpu: number;
    memory: number;
    pods: number;
  }> {
  ...
  private async getPodCount(namespace: string, deploymentName: string): Promise<number> {
    try {
      ...
      return pods.body.items.filter((pod: any) =>
        pod.status?.phase === 'Running' &&
        pod.status?.conditions?.some((cond: any) => cond.type === 'Ready' && cond.status === 'True')
      ).length;
    } catch (error) {
  ...
  private async waitForRollout(namespace: string, deploymentName: string, timeoutMs: number = 300000): Promise<void> {
