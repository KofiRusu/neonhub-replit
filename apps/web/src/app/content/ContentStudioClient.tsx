...
import { useEditorialOverview } from "@/src/hooks/useEditorial"
import { buildArticleJsonLd } from "@/src/lib/seo/jsonld"

type ContentPlanStatus = "draft" | "scheduled" | "review" | "published"

type ContentPlan = {
  id: string
  title: string
  type: "blog" | "email" | "social" | "product" | "case-study" | "video"
...
const STATUS_MAP: Record<string, ContentPlanStatus> = {
  planned: "scheduled",
  scheduled: "scheduled",
  review: "review",
  in_review: "review",
...
const DEFAULT_STATUS: ContentPlanStatus = "draft"
...
function ContentCard({ content, isSelected, onSelect, onEdit, onDelete, onDuplicate }: ContentCardProps) {
  ...
  const statusConfig: Record<ContentPlanStatus, { label: string; color: string; bg: string; border: string; icon: typeof CheckCircle }> = {
    published: { label: "Published", color: "text-neon-green", bg: "bg-neon-green/20", border: "border-neon-green/30", icon: CheckCircle },
    draft: { label: "Draft", color: "text-gray-300", bg: "bg-gray-500/20", border: "border-gray-500/30", icon: Edit },
    scheduled: { label: "Scheduled", color: "text-neon-blue", bg: "bg-neon-blue/20", border: "border-neon-blue/30", icon: Clock },
    review: { label: "In Review", color: "text-yellow-300", bg: "bg-yellow-400/20", border: "border-yellow-400/30", icon: AlertCircle },
  }

  const typeConfig = {
    blog: { icon: BookOpen, label: "Blog Post" },
    email: { icon: MessageSquare, label: "Email" },
    social: { icon: Hash, label: "Social" },
    product: { icon: Tag, label: "Product Copy" },
    "case-study": { icon: BarChart3, label: "Case Study" },
    video: { icon: Video, label: "Video Script" },
  }

  const statusMeta = statusConfig[content.status] ?? statusConfig[DEFAULT_STATUS]
  const typeMeta = typeConfig[content.type] ?? typeConfig.blog
  const StatusIcon = statusMeta.icon
  const TypeIcon = typeMeta.icon
  const publishLabel = content.publishAt ? new Date(content.publishAt).toLocaleString() : undefined
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
            ...
            <div className="flex items-center gap-2 mt-1 text-xs text-gray-400">
              ...
              {content.intent && (
                <>
                  ...
                  <span>Intent: {content.intent}</span>
                </>
              )}
            </div>
          </div>
        ...
        <div className="flex items-center space-x-2">
          <span
            className={`px-2 py-1 text-xs font-medium rounded-full border ${statusMeta.bg} ${statusMeta.border} ${statusMeta.color} flex items-center space-x-1`}
          >
      ...
      <div className="space-y-3 text-sm text-gray-300">
        {content.personaName && (
          <div className="flex items-center gap-2">
            ...
            <span>
              Persona: <span className="text-white">{content.personaName}</span>
            </span>
          </div>
        )}
        {publishLabel && (
          <div className="flex items-center gap-2">
            ...
            <span>Publish on {publishLabel}</span>
          </div>
        )}
        {content.summary && <p className="text-gray-400 line-clamp-2">{content.summary}</p>}
        {content.personaSummary && !content.summary && (
          <p className="text-gray-500 line-clamp-2">{content.personaSummary}</p>
        )}
      </div>
      ...
      <div className="mt-4 flex items-center justify-between text-xs text-gray-400">
        <span>Updated {new Date(content.updated).toLocaleDateString()}</span>
        {content.priority !== undefined && content.priority !== null && (
          <span className="text-neon-purple">Priority {content.priority}</span>
        )}
      </div>
    </motion.div>
  )
...
export default function ContentStudioClient({ baseUrl }: ContentStudioClientProps) {
  ...
  const { personas, personaOptions, keywords, calendar } = useEditorialOverview(
    selectedPersona === "all" ? undefined : selectedPersona
  )

  const contentPlans = useMemo<ContentPlan[]>(() => {
    if (!calendar.data) return []
    const keywordList = keywords.data ?? []
    return calendar.data.map((entry) => {
      const relatedKeywords = keywordList.filter((keyword) => keyword.personaId === entry.personaId)
      const primaryKeyword = relatedKeywords[0]

      return {
        id: `entry-${entry.id}`,
        title: entry.title,
        type: "blog",
        status: STATUS_MAP[entry.status ?? ""] ?? DEFAULT_STATUS,
        created: entry.createdAt,
        updated: entry.updatedAt,
        author: entry.persona?.name ?? "Editorial Team",
        publishAt: entry.publishAt,
        personaName: entry.persona?.name,
        personaSummary: entry.persona?.summary ?? null,
        summary: primaryKeyword ? `Focus: ${primaryKeyword.term}` : undefined,
        intent: primaryKeyword?.intent,
        tags: relatedKeywords.map((keyword) => keyword.term),
        priority: entry.priority ?? null,
      } satisfies ContentPlan
    })
  }, [calendar.data, keywords.data])

  useEffect(() => {
    if (selectedContent && !contentPlans.some((plan) => plan.id === selectedContent)) {
      setSelectedContent(null)
    }
  }, [selectedContent, contentPlans])
  ...
  const filteredContent = useMemo(() => {
    ...
    return contentPlans.filter((plan) => {
      const matchesSearch =
        ...
        (plan.intent && plan.intent.toLowerCase().includes(query))
      const matchesType = typeFilter === "all" || plan.type === typeFilter
      const matchesStatus = statusFilter === "all" || plan.status === statusFilter
      return matchesSearch && matchesType && matchesStatus
    })
  }, [contentPlans, searchQuery, typeFilter, statusFilter])

  const isEditorialLoading = personas.isLoading || keywords.isLoading || calendar.isLoading
  const loadError = (personas.error as Error | null) ?? (keywords.error as Error | null) ?? (calendar.error as Error | null)

  const articleJsonLd = useMemo(() => {
    if (contentPlans.length === 0) return undefined
    const primary = contentPlans[0]
    const description =
      primary.summary ??
      primary.personaSummary ??
      `Planned content for ${primary.personaName ?? "NeonHub audiences"}.`

    return JSON.stringify(
      buildArticleJsonLd({
        title: primary.title,
        description,
        url: `${baseUrl.replace(/\/$/, "")}/content`,
        datePublished: primary.created,
        dateModified: primary.updated,
        author: primary.author,
        keywords: primary.tags,
      })
    )
  }, [contentPlans, baseUrl])
  ...
  return (
    <>
      ...
      <div className="space-y-8">
          ...
          <div className="flex-1 flex gap-4">
            <select
              ...
              onChange={(e) => {
                ...
                setSelectedPersona(value === "all" ? "all" : Number(value))
              }}
        ...
        {/* Data state */}
        {isEditorialLoading && (
          <div className="glassmorphism-effect p-6 rounded-lg text-center text-gray-400">
            Loading keyword strategy data...
          </div>
        )}

        {loadError && !isEditorialLoading && (
          <div className="glassmorphism-effect p-6 rounded-lg text-center text-red-400">
            {loadError.message || "Failed to load editorial data"}
          </div>
        )}
        ...
        <AnimatePresence mode="wait">
          {activeView === "library" && (
            ...
            >
              {filteredContent.length === 0 && !isEditorialLoading && !loadError && (
                <div className="col-span-full glassmorphism-effect p-6 text-center text-gray-400">
                  No content plans match your filters.
                </div>
              )}
