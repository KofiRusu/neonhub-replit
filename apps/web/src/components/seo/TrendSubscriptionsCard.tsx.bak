...
export function TrendSubscriptionsCard({ className }: TrendSubscriptionsCardProps) {
  ...
  return (
    <Card className={className ?? 'border-border/60 bg-muted/20'}>
      ...
      <CardContent className="space-y-3">
        ...
        {subscriptions?.map((subscription) => {
          ...
          return (
            <div
              key={subscription.id}
              className="rounded border border-border/50 bg-background/60 p-3 text-sm text-muted-foreground"
            >
              <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                <div className="space-y-1">
                  <p className="font-medium text-foreground">#{subscription.id.slice(0, 8)}</p>
