import { RegulationType, RegulationFramework, RetentionPolicy } from '../types';

export class RegulatoryFrameworks {
  private static frameworks: Map<RegulationType, RegulationFramework> = new Map();
  ...
  private static validateConsent(consent: any, framework: RegulationFramework): boolean {
    for (const requirement of framework.consentRequirements) {
      switch (requirement) {
        case 'freely_given':
          if (!consent.freelyGiven) return false;
          break;
        case 'specific':
          if (!consent.specific) return false;
          break;
        case 'informed':
          if (!consent.informed) return false;
          break;
        case 'unambiguous':
          if (!consent.unambiguous) return false;
          break;
        case 'withdrawable':
          if (!consent.withdrawable) return false;
          break;
      }
    }
    return true;
  ...
  private static validateRetention(data: any, framework: RegulationFramework): boolean {
    const now = Date.now();
    for (const [key, policy] of Object.entries(framework.retentionRules)) {
      const retentionPolicy = policy as RetentionPolicy;
  ...
  private static validateDataSubjectRights(data: any, framework: RegulationFramework): boolean {
