...
export class PolicyEngine extends EventEmitter {
  ...
  async loadPolicy(policy: Policy): Promise<void> {
    try {
      ...
      throw new GovernanceError(
        `Failed to load policy ${policy.id}: ${(error as Error).message}`,
        'POLICY_LOAD_ERROR',
        { policyId: policy.id }
      );
    }
   ...
   */
  async evaluate(subject: AuditSubject, context?: Record<string, any>): Promise<{
    allowed: boolean;
    ...
    try {
      ...
      if (this.auditLogger) {
        await this.auditLogger.log({
          ...
          details: { error: (error as Error).message, context }
        });
      }
      throw error;
    }
   ...
   */
  private getApplicablePolicies(subject: AuditSubject): Policy[] {
    return Array.from(this.policies.values()).filter(policy => {
      // Check if policy is active
      if (policy.status !== 'active') return false;

      // Check jurisdiction overlap
      const jurisdictionOverlap = policy.jurisdiction.some(jur =>
        this.config.jurisdictions.includes(jur)
      );
      if (!jurisdictionOverlap) return false;
  ...
  ): Promise<{ allowed: boolean; violations: PolicyViolationError[] }> {
    const violations: PolicyViolationError[] = [];
    ...
    for (const rule of policy.rules) {
      ...
      try {
        ...
        if (!ruleResult.allowed) {
          policyAllowed = false;
          violations.push(new PolicyViolationError(
            policy.id,
            rule.id,
            {
  ...
  private evaluateUserCondition(
    ...
    if (subject.type !== SubjectType.USER) return { allowed: true };
   ...
   */
  private evaluateOperator(operator: ConditionOperator, left: any, right: any): { allowed: boolean } {
    switch (operator) {
  ...
  private validatePolicy(policy: Policy): void {
    ...
    for (const rule of policy.rules) {
      if (!rule.id || !rule.condition || !rule.action) {
        throw new GovernanceError(
          `Invalid rule structure in policy ${policy.id}`,
          'INVALID_RULE'
        );
      }
    }
   ...
   */
  private checkPolicyConflicts(policy: Policy, excludeId?: string): void {
    for (const [id, existingPolicy] of this.policies) {
      if (excludeId && id === excludeId) continue;

      // Check for conflicting jurisdictions and categories
      const jurisdictionOverlap = policy.jurisdiction.some(jur =>
        existingPolicy.jurisdiction.includes(jur)
      );
