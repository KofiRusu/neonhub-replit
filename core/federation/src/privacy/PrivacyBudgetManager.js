...
class PrivacyBudgetManager {
     ...
     */
    registerParticipant(nodeId, initialBudget) {
     ...
     */
    canAffordOperation(nodeId, requiredEpsilon) {
     ...
     */
    consumeBudget(nodeId, epsilon, delta = 0) {
        const budget = this.participantBudgets.get(nodeId);
        if (!budget) {
     ...
     */
    resetBudget(nodeId, newMaxBudget) {
        const budget = this.participantBudgets.get(nodeId);
        if (budget) {
            budget.usedBudget = 0;
     ...
     */
    getLowBudgetParticipants(thresholdRatio = 0.8) {
        const lowBudget = [];
