...
export class ModelValidation {
   ...
   */
  validateModelUpdate(modelUpdate: ModelUpdate): { isValid: boolean; reason?: string } {
    try {
  ...
  detectPoisoning(modelUpdates: ModelUpdate[], globalModel?: ModelUpdate): PoisoningDetectionResult[] {
    const results: PoisoningDetectionResult[] = [];
   ...
   */
  private calculateLayerStatistics(modelUpdates: ModelUpdate[]): Record<string, { mean: number; std: number; median: number }> {
    const layerStats: Record<string, { mean: number; std: number; median: number }> = {};
  ...
  private analyzeModelForPoisoning(
    ...
    layerStats: Record<string, { mean: number; std: number; median: number }>,
