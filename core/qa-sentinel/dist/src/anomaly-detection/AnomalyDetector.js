...
export class AnomalyDetector {
    constructor(threshold = 3.0, windowSize = 100) {
        ...
        this.windowSize = 100; // Number of data points to keep for analysis
    ...
    analyzeMetric(metricKey, values, telemetryData) {
        const anomalies = [];
        if (values.length < 10)
            return anomalies; // Need minimum data for analysis
        const historical = this.historicalData.get(metricKey) || [];
    ...
    calculateSeverity(zScore, actualValue, expectedValue) {
        if (zScore > 5)
            return 'critical';
        if (zScore > 4)
            return 'high';
    ...
    classifyAnomalyType(metric, actualValue, expectedValue) {
    ...
    detectTrendAnomalies(telemetryData, windowMinutes = 60) {
        const anomalies = [];
        const now = new Date();
