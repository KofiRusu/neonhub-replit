...
export class AutoScalingService extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      this.logger.error('Failed to start auto-scaling service', error as Error);
      throw new GlobalOrchestratorError(
        GlobalOrchestratorErrorCode.SCALING_FAILED,
        'Failed to start auto-scaling service',
        undefined,
        undefined,
  ...
  async stop(): Promise<void> {
    ...
    try {
      ...
      this.logger.error('Error stopping auto-scaling service', error as Error);
    }
  }
  ...
  private async evaluateScalingNeeds(): Promise<void> {
    try {
      ...
      this.logger.error('Error evaluating scaling needs', error as Error);
    }
  }

  private async analyzeScalingConditions(): Promise<ScalingDecision[]> {
  ...
  private async executeScalingDecision(decision: ScalingDecision): Promise<void> {
    try {
      ...
      this.logger.error('Failed to execute scaling decision', error as Error);
      throw new GlobalOrchestratorError(
        GlobalOrchestratorErrorCode.SCALING_FAILED,
        'Failed to execute scaling decision',
        undefined,
        undefined,
        { decision, originalError: (error as Error).message }
      );
    }
  }

  private async scaleUp(targetNodes: string[]): Promise<void> {
  ...
  private async scaleDown(targetNodes: string[]): Promise<void> {
  ...
  async manualScale(action: ScalingAction, targetNodes: string[], reason: string): Promise<void> {
    const decision: ScalingDecision = {
      action,
      targetNodes,
      reason,
  ...
  getScalingHistory(limit: number = 50): ScalingDecision[] {
