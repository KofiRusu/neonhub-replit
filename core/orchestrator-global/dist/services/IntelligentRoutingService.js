...
export class IntelligentRoutingService extends EventEmitter {
    constructor(config, logger) {
        ...
        this.nodeLoadCache = new Map();
        this.geographicRoutingTable = new Map();
    ...
    buildRoutingTable(nodes) {
        // Group nodes by federation
        const nodesByFederation = new Map();
        ...
        for (const [federationId, federationNodes] of nodesByFederation) {
            const decisions = federationNodes.map(node => ({
    ...
    buildGeographicRoutingTable(nodes) {
    ...
    determineTargetFederation(message) {
    ...
    weightedRoundRobinRouting(routingDecisions) {
    ...
    updateAdaptiveMetrics(nodeId, message) {
