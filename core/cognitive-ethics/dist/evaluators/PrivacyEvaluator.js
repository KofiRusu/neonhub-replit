...
export class PrivacyEvaluator {
    ...
    async evaluate(context) {
        ...
        if (this.policy.privacy.piiDetectionEnabled) {
            ...
            const unauthorizedPII = detections.filter(d => !allowedTypes.includes(d.type));
            if (unauthorizedPII.length > 0) {
                violations.push({
                    type: 'pii_leak',
                    details: `Detected ${unauthorizedPII.length} unauthorized PII types: ${unauthorizedPII.map(p => p.type).join(', ')}`,
        ...
        if (this.policy.privacy.differentialPrivacyEnabled) {
            ...
            const result = this.checkPrivacyBudget(budgetKey, this.policy.privacy.privacyBudgetEpsilon);
     ...
     */
    checkPrivacyBudget(key, epsilon) {
     ...
     */
    async evaluateAndRedact(context) {
...
//# sourceMappingURL=PrivacyEvaluator.js.map