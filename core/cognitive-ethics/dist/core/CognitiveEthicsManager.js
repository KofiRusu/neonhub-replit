...
export class CognitiveEthicsManager {
     ...
     */
    async evaluate(context, input, output) {
        const evaluationId = this.generateEvaluationId();
        const timestamp = new Date().toISOString();
        // Run all evaluators in parallel
        const [fairness, safety, privacy,] = await Promise.all([
        ...
        const attestation = this.generateAttestation(evaluationId, context, overallPassed);
        return {
            evaluationId,
            timestamp,
     ...
     */
    async postCheck(context, input, output) {
     ...
     */
    generateAttestation(evaluationId, context, passed) {
...
//# sourceMappingURL=CognitiveEthicsManager.js.map