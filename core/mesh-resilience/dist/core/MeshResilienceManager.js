...
class MeshResilienceManager extends eventemitter3_1.EventEmitter {
     ...
     */
    async performOperation(type, data) {
        const operation = {
    ...
    async selfReconstruct() {
        ...
        const healthyNodes = Array.from(this.meshNodes.values()).filter((node) => node.health.status === 'online');
        if (healthyNodes.length < this.config.selfReconstructionConfig.minHealthyNodes) {
     ...
     */
    async executeRecoveryPlaybook(playbookId) {
     ...
     */
    handlePartition(partitionedNodes) {
        this.isOnline = false;
     ...
     */
    async propagateOperation(operation) {
     ...
     */
    async requestStateFromNodes(nodes) {
     ...
     */
    async reconstructFromStates(states) {
     ...
     * Apply reconstructed state
     */
    async applyReconstructedState(state) {
        if (state) {
     ...
     */
    async rollbackRecoverySteps(results) {
     ...
     */
    loadDefaultRecoveryPlaybooks() {
        // Add default playbooks for common scenarios
        const nodeFailurePlaybook = {
